...
Version `0.2.5` build `1` moves menu-bar Start, Remove Helper, and Quit confirmations into an application-owned native presenter, so confirmed actions survive transient MenuBarExtra teardown. Lease start and renewal run on a serial monotonic coordinator, independent of full UI inspection and the main run loop. Root-owned, app-readable terminal generation tombstones prevent a late lease rewrite or rapid replug from rearming an ended session.
...
The DMG and checksum are written to `dist/`. Packaging validates version `0.2.5` build `1`, helper version `1`, arm64 binaries, strict ad-hoc signatures, expected Gatekeeper rejection, checksum integrity, and that no app process was started or stopped.

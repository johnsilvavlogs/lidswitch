...
| containment owner lease / reap | `testContainmentClaimCarriesItsLeaseDeadlineIntoActualStoreTransition` uses the production store claim/CAS path with the claimed deadline and proves that an expired ambiguous receipt can be reclaimed only in its observation-only phase; `testExpiredUnprovenLeaderReapRecoversOnlyAfterExactKernelExtinction` keeps ECHILD non-authoritative, preserves the fence for live evidence, and permits retirement only after exact member birth tuples plus two stable PGID/SID inventories prove kernel extinction. The queued scheduler call graph is source-inspected; isolated queue timing remains runtime proof |

...
1. The user prepares helper version `7` in the root-owned `Current` directory; legacy login and shell-helper artifacts are removed while protection remains off.
...
Activation is currently qualified only for macOS build `25F84`. The packaged app includes `CFBundleShortVersionString=0.2.12`, `CFBundleVersion=7`, and helper version `7` under `Contents/Library/LaunchServices`; the installed helper is exposed through the authenticated raw-XPC Mach service, never a lease `WatchPaths` trigger.

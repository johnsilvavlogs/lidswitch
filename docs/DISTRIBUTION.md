...
LidSwitch `0.2.3` build `5` is distributed as a public manual DMG. It is ad-hoc signed, not Developer ID signed, and not notarized. Recipients must expect Gatekeeper's **Open Anyway** flow.
...
4. Confirm the bundle reports app version `0.2.3`, build `5`, helper version `5`, arm64, and strict ad-hoc signature validity.

...
class HostedEvidenceFixtureTests(unittest.TestCase):
    def make_fixture(self):
        ...
        policy_desc = descriptor(root / "orchestration/policy.json", "hosted-runner-policy.json")
        authority = {"schema": "lidswitch-hosted-authority-ledger-v1", "policy": policy_desc,
                     "source": {"commit": COMMIT, "tree": "d86650eccfe3326fc968fc855a07a1e3d06aaf57", "root": directory_descriptor(),
                                ...
                                "manifest_descriptor": descriptor(root / "source/source_snapshot_manifest.jsonl", "script/source_snapshot_manifest.jsonl")},
                     ...
                     "generated": {"entry": descriptor(root / "authority/entry.py", "hosted-held-entry.py"), "contract": descriptor(root / "authority/contract.json", "hosted-held-contract.json"), "root": directory_descriptor()}}
        ...
        write(root / "authority/live-envelope.json", {"receipt_sha256": meta(root / "authority/live-state-retained.receipt")["sha256"], "preflight_sha256": meta(root / "authority/preflight-state.snapshot")["sha256"], "postflight_sha256": meta(root / "authority/postflight-state.snapshot")["sha256"]}, 0o400)
        prepare = {"schema": "lidswitch-hosted-prepare-v2", "authority": "/tmp/a", "ledger": descriptor(root / "authority/ledger.json", "hosted-authority-ledger.json"), "entry": descriptor(root / "authority/entry.py", "hosted-held-entry.py"), "contract": descriptor(root / "authority/contract.json", "hosted-held-contract.json"), "source_manifest_sha256": meta(root / "source/source_snapshot_manifest.jsonl")["sha256"]}
        write(root / "receipts/prepare.json", prepare, 0o400)
        retained = {"live-state-retained.receipt": descriptor(root / "authority/live-state-retained.receipt", "live-state-retained.receipt"), "preflight-state.snapshot": descriptor(root / "authority/preflight-state.snapshot", "preflight-state.snapshot"), "postflight-state.snapshot": descriptor(root / "authority/postflight-state.snapshot", "postflight-state.snapshot"), "hosted-live-envelope.json": descriptor(root / "authority/live-envelope.json", "hosted-live-envelope.json")}
        build = {"schema": "lidswitch-hosted-build-v2", "source": authority["source"], "generated": authority["generated"], "ledger": prepare["ledger"], "entry": prepare["entry"], "contract": prepare["contract"], "retained": retained, "release_output": "/tmp/release"}
        ...
        inputs = {"appSourceSeal": "3" * 64, "baseManifestSHA256": meta(root / "source/source_snapshot_manifest.jsonl")["sha256"], "generatedAnchorSHA256": anchor["sha256"], "helperSourceSeal": "4" * 64, "releaseIdentitySHA256": "5" * 64, "trustAnchorTemplateSHA256": "6" * 64}
        ...
        release = {"seal_sha256": "b" * 64, "build_receipt_sha256": meta(root / "release-output/build-receipt.json")["sha256"], "anchor_sha256": anchor["sha256"], "anchor_size": anchor["size"], "source_manifest_sha256": inputs["baseManifestSHA256"], "release_identity_sha256": inputs["releaseIdentitySHA256"], "app": release_receipt["artifacts"]["app"], "helper": release_receipt["artifacts"]["helper"]}

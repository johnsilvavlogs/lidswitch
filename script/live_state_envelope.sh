...
LIDSWITCH_CANDIDATE_INACTIVE_NONE_REASONS="legacy-migration legacy-migration-superseded pristine"
LIDSWITCH_CANDIDATE_TERMINAL_SESSION_REASONS="ac-disconnect activation-failed activation-publication-failed activation-verification-failed confirmed-power-unknown drift expired helper-restart install-recovery legacy-migration legacy-restore peer-process-invalid peer-restore reconnect-expired reconnect-power-drift renewal-publication-failed replay-or-durability-denial startup-recovery uninstall-recovery unproven-schema2-restore user-end user-restore-recovery"

...
swift_sandbox_verified_python() {
  ...
  case "$helper" in
    safe-file)
      fd=34
      expected="1efa793bfd62d718addaa4bf588e3ba11c1e7b2d601f60b877f60801afddb96c"
      size=118213
      ;;
    supervisor)
      fd=35
...
swift_sandbox_reject_inherited_paths() {
  ...
  for name in \
    ...
    LIDSWITCH_SWIFT_BENCHMARK_REQUESTED_OUTPUT LIDSWITCH_SWIFT_BENCHMARK_DEST_ID LIDSWITCH_SWIFT_BENCHMARK_PARENT LIDSWITCH_TEST_FIXTURE_ROOT \
...
swift_sandbox_capture_benchmark_contract() {
  ...
  if [[ -z "$output" && -z "$samples" && -z "$app" ]]; then
    ...
    LIDSWITCH_SWIFT_BENCHMARK_PARENT="$LIDSWITCH_SWIFT_EXEC_ROOT"
  ...
  LIDSWITCH_SWIFT_BENCHMARK_PARENT="$app_parent"
...
swift_sandbox_render_profile() {
  ...
  /usr/bin/sed \
    ...
    -e "s|@BENCHMARK_PARENT@|$LIDSWITCH_SWIFT_BENCHMARK_PARENT|g" \
  ...
  ! /usr/bin/grep -Eq '@(EXEC_ROOT|CONTROL_ROOT|REPO_ROOT|SOURCE_ROOT|HELPER_SOURCE_ROOT|APP_SOURCE_ROOT|REAL_HOME|BENCHMARK_APP|BENCHMARK_PARENT|BENCHMARK_HELPER|CLT_ROOT|SDKROOT|SWIFT_TOOL|SWIFTC_TOOL|SWIFT_FRONTEND_TOOL|CLANG_TOOL|CLANGXX_TOOL|LD_TOOL|DSYMUTIL_TOOL|XCODE_[A-Z_]+)@' "$LIDSWITCH_SWIFT_SANDBOX_PROFILE" || return 64
}
